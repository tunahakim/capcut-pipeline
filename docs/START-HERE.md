...
Ngưỡng quyết định giữa đọc trọn file và trích dòng, bổ sung cho luật trích dòng ở trên. Đo bằng **số dòng**, không đo bằng byte, vì thứ quyết định giá trị của việc trích là bao nhiêu dòng loại được chứ không phải file nặng bao nhiêu. Ba bậc, tính trên số dòng thật của file: dưới **250 dòng** thì **luôn đọc trọn**, bất kể tiết kiệm bao nhiêu; từ **250 đến 600 dòng** thì chỉ trích khi loại được từ **50 phần trăm** trở lên; trên **600 dòng** thì hạ ngưỡng về **30 phần trăm**. File **không nằm trong repo** thì luôn phải trích, vì trợ lý không fetch được. Quyết định đặt trong script chứ không đặt ở người: script đếm số dòng giữ lại, tự tính tỉ lệ, tự chuyển sang in trọn khi không đạt ngưỡng, và in rõ đã chọn nhánh nào cùng vì sao. Lý do đổi, đo ngày 03/08/2026: trích `bgblur_frames.py` 154 dòng chỉ loại được 5 dòng tức 3,2 phần trăm, còn `shots_dump.py` 148 dòng loại được 6,1 phần trăm, trong khi mỗi lượt trích tốn thêm một script, một vòng đối đáp và làm mất nguyên văn phần bị bỏ; đọc trọn mã thường trả lời được nguyên nhân ngay trong một lượt, và một lượt hỏi lại đắt hơn vài KB token rất nhiều.
