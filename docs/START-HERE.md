...
Thứ tự bắt buộc cho một phiên mới, đọc trọn bốn file: `README.md`, rồi file này, rồi `STATE.md`, rồi `TODO.md`. Mọi file còn lại **chỉ đọc khi việc đang làm cần tới nó**, và đọc ngay trước lúc dùng chứ không nạp sẵn từ đầu phiên: `reference.md` khi cần hằng số và công thức, `failures.md` trước khi kết luận về một hành vi lạ, `research-log/INDEX.md` cùng nhật ký một hoặc hai phiên gần nhất khi cần biết vì sao mọi thứ thành ra thế này, `model.md` khi cần hiểu vì sao phải làm thế, `procedures.md` khi cần quy trình, `scripts.md` khi cần biết script nào làm gì, `reference-catalog.md` khi cần tra hiệu ứng hoặc cú pháp lệnh. `legacy/v0.8-full.md` chỉ khi các file kia thiếu, và khi đó tìm mục cụ thể chứ đừng đọc tuần tự.
