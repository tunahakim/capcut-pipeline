...
docs/
  ...
  architecture.md    <- hop dong kien truc: bon tang, hai cong, nguon su that
  research-log/      <- thi qua khu: nhat ky theo phien, xem INDEX.md
  legacy/v0.8-full.md  <- 299 KB, ban luu tru
```

Thứ tự bắt buộc cho một phiên mới, đọc trọn bốn file: `README.md`, rồi file này, rồi `STATE.md`, rồi `TODO.md`. Mọi file còn lại **chỉ đọc khi việc đang làm cần tới nó**, và đọc ngay trước lúc dùng chứ không nạp sẵn từ đầu phiên: `reference.md` khi cần hằng số và công thức, `failures.md` trước khi kết luận về một hành vi lạ, `research-log/INDEX.md` cùng nhật ký một hoặc hai phiên gần nhất khi cần biết vì sao mọi thứ thành ra thế này, `model.md` khi cần hiểu vì sao phải làm thế, `procedures.md` khi cần quy trình, `scripts.md` khi cần biết script nào làm gì, `reference-catalog.md` khi cần tra hiệu ứng hoặc cú pháp lệnh, `architecture.md` khi cần biết tầng nào được phụ thuộc tầng nào và cái gì được cấu hình. `legacy/v0.8-full.md` chỉ khi các file kia thiếu, và khi đó tìm mục cụ thể chứ đừng đọc tuần tự.

Luật ba file, để tài liệu không phình và không tự mâu thuẫn: file này giữ thứ **không đổi theo phiên**; `STATE.md` là **ảnh chụp thì hiện tại**, sửa bằng cách ghi đè chứ không thêm vào; `TODO.md` là **thì tương lai**, xong việc thì xoá khỏi đó chứ không đánh dấu hoàn thành; `research-log/` là **thì quá khứ**, chỉ ghi thêm, không sửa lặng lẽ. Cùng một sự thật không được chép ra hai chỗ. Mỗi file nhật ký khai hai dòng ngay dưới tiêu đề H1: một dòng tóm tắt một câu, và một dòng khai giờ cùng buổi của phiên dạng `23:13 khuya`. Hai dòng đó là nguồn sự thật để `tools/rlog_index.py` sinh lại bảng, nhờ vậy `docs/research-log/INDEX.md` không còn là bản chép thứ hai.
...
Gộp nhiều bước vào một lượt, đánh số rõ ràng. Bước nào làm thay đổi (ghi file, cài đặt, commit, đổi cấu hình) thì kèm **dấu hiệu đạt**: output mong đợi, hoặc lệnh kiểm tra và kết quả đúng của nó; bước chỉ đọc thì không cần. Mỗi lệnh kèm một dòng **gửi gì**: đạt thì gửi dòng tổng kết cùng các con số đã dự đoán trước, hỏng thì gửi khối lỗi cộng mười dòng ngay trước nó. Người dùng đối chiếu: khớp thì chạy tiếp, khác thì dừng và báo lại số bước cùng output thực tế. Lý do gộp: mỗi lượt đối đáp gửi lại toàn bộ hội thoại, chia nhỏ việc là tự đốt ngân sách. Chỉ dừng chờ khi chưa xem output thật thì không thể viết được lệnh kế tiếp (lệnh cần giá trị trả về, cần chẩn đoán lỗi, hoặc rẽ nhánh). Riêng "bước sau chạy sau bước trước" không phải lý do để tách lượt. Ngoại lệ: lệnh khó hoàn tác (xoá, ghi đè, force push, migration, sửa cấu hình hệ thống) tách riêng một lượt. Mỗi lượt nói rõ **đang ở đâu trong lộ trình**. Mỗi phiên nhắc người dùng kiểm tra đã `git pull --rebase` và đã push chưa.
...
Console in được tiếng Việt **có dấu**, với điều kiện script tự lo mã hoá: mọi script có thể in tiếng Việt phải gọi `sys.stdout.reconfigure` sang UTF-8 với `errors` là `replace`, bọc trong `try/except` ngay sau phần import, mọi `open()` phải khai mã hoá tường minh, và mọi lệnh `subprocess` bắt output của tiến trình con phải khai `encoding` UTF-8. Đo tám ca ngày 04/08/2026: in thẳng ra console đã sạch sẵn vì Python dùng `WriteConsoleW` bỏ qua code page, nhưng khi stdout bị pipe thì Python tụt về `cp1252` theo locale và chết bằng `UnicodeEncodeError` ở chữ có dấu đầu tiên. Muốn output qua pipe hiển thị đúng thì đặt `[Console]::OutputEncoding` sang UTF-8 trong phiên PowerShell; đó là chuyện hiển thị, không phải chuyện dữ liệu. Không đặt biến môi trường UTF-8 mode thường trú, vì nó đổi mã hoá mặc định của `open()` cho mọi script cùng lúc. Nội dung file luôn ghi UTF-8 không BOM. Không dùng `Out-File`. Mọi `Get-Content` đọc file trong repo phải kèm `-Encoding UTF8`. Script in nhiều thứ thì tự bọc phần cần chép giữa hai dòng đánh dấu.

Đưa lệnh chép-dán được ngay. Chỗ nào phải làm tay thì hướng dẫn chi tiết kể cả bấm chuột ở đâu.

**Chọn công cụ theo cỡ việc, đừng máy móc.** Sửa một hai dòng, đổi tên file, xoá file, chạy lệnh git: viết thẳng một hai dòng PowerShell trong hội thoại là đúng và gọn hơn hẳn việc sinh ra một file script chỉ để chạy một lần. Khuôn heredoc `@'...'@` cộng `WriteAllText` là **bắt buộc khi đoạn mã là Python**, khi nội dung có chữ tiếng Việt, hoặc khi thao tác đủ phức tạp để cần đếm khớp và kiểm trước khi ghi; nó không phải nghi thức áp cho mọi thứ. Ba điều không nới trong mọi trường hợp: không nhúng Python vào dòng lệnh PowerShell vì dấu gạch ngược và dấu nháy sẽ hỏng âm thầm, không dùng toán tử `>` vì nó ghi UTF-16LE, và mọi phép sửa file phải xác nhận lại kết quả sau khi sửa. Nguyên tắc chung của cả mục này: mục tiêu là ít lượt đối đáp và ít hỏng ngầm, luật nào đang làm tăng cả hai thì sửa luật chứ đừng tuân theo cho đúng hình thức.
...
**Đọc mã nguồn bằng `python tools/read_src.py <đường-dẫn> [--grep <từ khoá> ...]`**, đừng viết lại script trích trong hội thoại. Công cụ tự đếm số dòng thật, tự chọn giữa in trọn và trích, in rõ đã chọn nhánh nào cùng vì sao, in mỗi khoảng bị bỏ qua kèm số dòng đã bỏ, rồi ghi kết quả ra file UTF-8 trong `data\tmp\` và mở Notepad. Dấu khoảng bị bỏ qua là phần quan trọng nhất, vì nó ngăn việc kết luận nhầm rằng một đoạn mã không tồn tại. Ngưỡng ba bậc nằm trong mã của công cụ chứ không nằm ở người, và đo bằng **số dòng** chứ không bằng byte, vì thứ quyết định giá trị của việc trích là bao nhiêu dòng loại được chứ không phải file nặng bao nhiêu: dưới **250 dòng** luôn in trọn; từ **250 đến 600 dòng** chỉ trích khi loại được từ **50 phần trăm** trở lên; trên **600 dòng** hạ ngưỡng về **30 phần trăm**; file **không nằm trong repo** thì luôn trích vì trợ lý không fetch được. Lý do bỏ ngưỡng 4 KB cũ ghi ở `research-log/2026-08-03-2-frame-audit-va-luat-trich.md`.
...
Trước khi đề xuất bất kỳ bản vá nào, **khai báo lỗ hổng đọc của mình**. Fetch mỗi file **đúng một lần** theo cách ghi ở `README.md`, rồi đối chiếu byte bằng **đúng một lệnh** `python tools/repo_bytecheck.py`; sạch thì đi tiếp ngay chứ đừng tốn thêm lượt nào. Không viết nội dung thay thế cho một file mà mình không có nguyên văn, và không kết luận rằng một hàm hay một câu văn không tồn tại chỉ vì mình không nhìn thấy nó. **Đọc tài liệu ngay trước việc cần tới nó, đừng nạp cả loạt từ đầu phiên**: ngữ cảnh của trợ lý bị nén dần trong lúc làm việc, thứ nạp sớm là thứ mất nguyên văn trước, và nó mất đúng khúc giữa trong khi đầu và đuôi còn nguyên nên nhìn vẫn y như đã đọc đủ. Mất nguyên văn thì nói ra và xin trích lại, tuyệt đối đừng vá mò.

Nếu người dùng đề xuất hướng có vấn đề, nói thẳng. Nếu tự phát hiện mình sai, cũng nói thẳng. Không dùng emoji.

**Cái gì dùng lại thì thành script trong repo, không gõ lại trong hội thoại.** Một đoạn mã sắp phải viết lần thứ hai là một script còn thiếu: đặt nó vào `tools/`, tên nói rõ việc, docstring một dòng, rồi commit và push để máy kia dùng được ngay mà không phải chép qua chat. Trong hội thoại chỉ đưa **lệnh chạy**, không đưa lại thân mã. Ngược lại thì đừng đóng gói: thứ chạy đúng một lần trong đúng một phiên thì để ở `data\tmp\`, còn việc một hai dòng thì làm thẳng. Hai công cụ đã có theo luật này: `python tools/repo_bytecheck.py` đối chiếu working copy với blob GitHub và so `HEAD` với `main`, `python tools/read_src.py` đọc mã nguồn theo ngưỡng ba bậc.

Tạo file mới cũng bằng script, không bắt người dùng dán tay. Luật này áp dụng cho **cả file mã lẫn file tài liệu**: sinh nội dung bằng heredoc `@'...'@` rồi `WriteAllText` với `UTF8Encoding($false)`, đúng khuôn của script vá. Lý do không phải tiết kiệm công mà là **mã hoá**: `>` của PowerShell ghi ra UTF-16LE còn Notepad có thể lưu cp1252, nên dán tay một file tiếng Việt là một cơ hội hỏng mã hoá âm thầm mà `docs_audit.py` không bắt được, vì nó đếm byte chứ không kiểm mã hoá. Ngoại lệ duy nhất: khi trợ lý đã báo sắp hết ngữ cảnh thì được phép đưa văn bản thuần cho người dùng tự dán, tuy nhiên cực kỳ hạn chế việc này vì người dùng dán tay nếu lỗi thì việc fix còn tốn token hơn.
