...
**Hoàn tất `tools/rlog_index.py`: dòng khai Phiên và phép nghiệm thu.** Tool đã chạy được và lượt chèn ngược dòng tóm tắt cho 25 file cũ đã xong ngày 04/08. Còn ba phần. **CẢNH BÁO, đọc trước khi chạy:** bảng trong `docs/research-log/INDEX.md` là nguồn **duy nhất** của thông tin buổi, vì 26 trên 28 file nhật ký chưa có dòng khai Phiên. Chạy `python tools/rlog_index.py --apply` sẽ viết lại cả 27 dòng cột Phiên theo khuôn `04/08-4` và **xoá mất** phần buổi. Không chạy `--apply` trước khi làm xong phần Một. Bản chụp cứu hộ đã commit ở `artifacts/phien-buoi-truoc-khi-sinh-lai-INDEX-20260805.json`. Hằng số so khớp phải là `**Phiên:**` có dấu, khớp file `04/08-4`, không phải `**Phien:**`; file `05/08-1` từng viết không dấu và đã sửa. Cùng bệnh với `SUMMARY_KEY`, vốn là `**Tóm tắt:**` có dấu trong khi docstring của tool ghi không dấu. Một, chèn ngược dòng khai Phiên: buổi lấy từ cột Phiên của bảng hiện có, giờ lấy từ `CreationTime` của file **chỉ khi** ngày của `CreationTime` trùng ngày trong tên file, vì đo được rằng file cũ mang ngày của lần chép hoặc lần clone chứ không phải ngày viết; lệch thì để trống giờ chứ không bịa. Hai, tool ghép cột Phiên từ hai nguồn, phần số thứ tự suy từ tên file nên không ai gõ sai được, phần giờ và buổi đọc từ dòng khai, rồi sinh lại bảng. Ba, chỉ sau hai phần trên mới đo được tiêu chí xong: xoá tay một dòng trong bảng rồi chạy tool thì dòng đó hiện lại đúng nguyên văn, và không file nhật ký nào bị sửa thân bài.

**Rà toàn bộ script cho khớp luật mã hoá mới.** Phép đo tám ca ngày 04/08 đã bác hướng nâng PowerShell: nguyên nhân là Python tụt về `cp1252` theo locale khi stdout bị pipe, không phải shell, nên PowerShell 7 không sửa được khâu đó và không cần cài. Luật mới đã vào mục 8 của `docs/START-HERE.md`. Việc còn lại là rà từng script: file nào có thể in tiếng Việt phải gọi `sys.stdout.reconfigure` bọc `try/except` ngay sau phần import, và mọi `open()` phải khai `encoding` tường minh. Từ 05/08/2026 luật có ba chiều chứ không phải hai: ghi ra bằng `sys.stdout.reconfigure`, đọc file bằng `open()` khai mã hoá, và bắt output tiến trình con bằng `subprocess` cũng phải khai `encoding` UTF-8. Tiêu chí xong: một script trong `tools/` grep chứng minh không còn `open()` nào thiếu khai mã hoá và không còn lệnh `subprocess` nào bắt output mà thiếu `encoding`, và một script in tiếng Việt qua pipe không còn `UnicodeEncodeError`.

**Miễn trừ trần có hạn, thay cho việc nâng trần lặng lẽ.** Ngày 04/08 một lượt kết phiên đã nâng trần `docs/STATE.md` từ 15 lên 25 KB vì hết ngữ cảnh để rút gọn, và việc đó không để lại dấu vết nào máy đọc được. Làm một file `docs/budget-waivers.json` commit vào repo, mỗi mục khai file, lý do, ngày cấp, ngày hết hạn, cộng một khoá tự dặn rằng giải quyết xong thì xoá mục đi. `tools/docs_audit.py` phơi ra một hàm trả về trần cùng miễn trừ đang hiệu lực, `tools/docs_patch.py` gọi đúng hàm đó qua bảng tên nhập sẵn nên hai tool không bao giờ lệch nhau. Miễn trừ còn hạn thì in một khối riêng kèm lý do và hạn, không vào khối vấn đề nên mã thoát vẫn 0; quá hạn thì thành vấn đề thật; file đã tụt xuống dưới trần mà mục vẫn còn thì in một dòng báo mục thừa. Tiêu chí xong: bốn ca selftest mới gồm còn hạn, quá hạn, thừa, và không có file miễn trừ nào.

**Chuẩn hoá bốn file đầu phiên.** Bốn file `README.md`, `docs/START-HERE.md`, `docs/STATE.md` và `docs/TODO.md` là toàn bộ phần đọc đầu phiên của mọi trợ lý, và đang giữ lẫn nhiệm vụ của nhau: `docs/START-HERE.md` giữ số đo lẽ ra thuộc `docs/STATE.md` và đó là lý do nó ở 98 phần trăm trần, `docs/STATE.md` giữ câu kể diễn biến lẽ ra thuộc `docs/research-log/`, và nhiều số đo bị chép làm hai bản ở `docs/STATE.md` với `docs/TODO.md` rồi lệch nhau. Danh sách chỗ hỏng đã soát xong ngày 05/08 và nằm trong nhật ký phiên tương ứng. Cùng lượt đó ghi luật chọn op và quy trình đo neo trước khi vá vào chỗ nào rẻ nhất. Tiêu chí xong: không sự thật nào còn hai bản, và `docs/START-HERE.md` xuống dưới 90 phần trăm trần.
...
Chuyển docstring của khoảng 25 file `.py` còn viết tiếng Việt không dấu sang có dấu, vì mô tả không dấu rất khó đọc và repo đang lai hai kiểu. Khoảng chục file như `scripts_v1/clone_project.py` và `tools/data_manifest.py` đã có dấu và chạy sạch, nên đây là chuẩn hoá về bên đã kiểm chứng chứ không phải thử nghiệm mới. Giữ nguyên hậu tố `[KIEM: ...]` dạng ASCII vì bảng nhãn so chuỗi không dấu. Tiêu chí xong: `python tools/scripts_index.py --brief` không còn dòng mô tả nào thiếu dấu, và lượt ghi lại bảng chạy sạch.

Hai nợ nhỏ phát hiện 05/08/2026. `tools/docs_audit.py` bắt mảnh chuỗi định dạng trong mã Python thành đường dẫn thiếu, cân nhắc bỏ qua token có ký tự phần trăm liền trước. Dòng mô tả rút gọn của `tools/v4_mold.py` trong chế độ `--brief` lấy nhầm dòng cú pháp thứ hai chứ không phải câu mô tả.
