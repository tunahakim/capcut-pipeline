...
**Sửa 53 lỗi mã hoá và docstring còn lại mà `tools/py_audit.py` đo được.** Nhóm 42 chỗ thiếu `sys.stdout.reconfigure` đã đóng ngày 05/08/2026, và `--exclude` đã có nên `_deprecated/` không còn làm nhiễu số đo; số hiện hành ở `docs/STATE.md`. Hai nhóm còn lại. Một, **32 docstring không dấu**: lấy nguyên liệu bằng `python tools/docstring_dump.py --scope tools/` rồi `--scope scripts_v1/` và `--scope pipeline/`, viết lại bằng tiếng Việt **có dấu** và **không hard wrap**, mỗi ý một dòng dài, ghi ngược vào file bằng `tools/docs_patch.py` với `content_file`; giữ nguyên hậu tố `[KIEM: ...]` ở dòng cuối docstring vì `tools/scripts_index.py` đọc nhãn đó. Hai, **21 chỗ `subprocess` bắt output thiếu `encoding`**: phải đọc mắt từng chỗ chứ tuyệt đối không vá loạt, vì khai `encoding` là bật chế độ văn bản nên chỗ nào đang tự `.decode()` sẽ chết bằng `AttributeError`, và `tools/frame_audit.py` dòng 87 dùng `stdout=subprocess.PIPE` với ffmpeg có thể đang nhận dữ liệu nhị phân nên thêm `encoding` vào đó sẽ phá phép đo pixel. Tiêu chí xong: `python tools/py_audit.py --brief` cho 0 `LOI`, và một script in tiếng Việt qua pipe không còn lỗi. Gộp luôn nợ `reference/describe.json` đang là UTF-16LE có BOM: grep xem script nào đọc nó, chuyển sang UTF-8 không BOM, rồi chạy lại chính script ấy.
...
Thêm khoá `end_mode` cho op `replace_between` của `tools/docs_patch.py`, và **đảo mặc định sang không nuốt neo cuối**. Lý do ở `docs/STATE.md`: một spec quên chép `end` vào `new` đã xoá mất ba đoạn của `docs/STATE.md` mà không ai thấy. Người dùng đã chốt hướng: thừa thì dễ sửa, mất thì khoai. Giá phải trả là bốn ca selftest `between-duong`, `between-end-khop-nhieu`, `between-lech-byte` và `do-vung-mot-duong` đều neo vào ngữ nghĩa cũ nên phải sửa cùng lượt, và đoạn docstring nói vùng bị thay gồm cả hai sentinel thành sai. Tiêu chí xong: mặc định giữ lại `end`, `end_mode` bằng `gom` khôi phục hành vi cũ, `--probe` in dòng đầu và dòng cuối của vùng sắp mất kèm một dòng nói rõ `end` có xuất hiện lại trong `new` hay không ở mức `CANH BAO`, và selftest có thêm một ca âm gồm hai edit mà edit trước xoá neo của edit sau.
...
`tools/docs_patch.py` có `--probe` đo từng edit độc lập trên bản gốc trong khi `apply_edits()` áp tuần tự, nên spec nhiều edit trên cùng một file có thể probe sạch rồi trượt ở bước ghi khi edit trước xoá mất neo của edit sau; đo được ngày 05/08/2026 ở lượt vá kết phiên. Tiêu chí xong: `run_probe()` dựng bản mới tuần tự trong bộ nhớ dùng chung đường với `apply_edits()`, cộng một ca selftest âm gồm hai edit mà edit trước xoá neo của edit sau, kỳ vọng probe báo hỏng.
