...
**Viết lại 16 docstring không dấu còn lại mà `tools/py_audit.py` đo được.** Hai nhóm kia đã đóng ngày 05/08/2026: 42 chỗ thiếu `sys.stdout.reconfigure`, và 21 chỗ `subprocess` bắt output thiếu `encoding` đóng bằng 6 chỗ thêm `encoding` thật cộng 15 chỗ khai nhãn pragma. Lô 15 docstring trong `tools/` đã viết lại, và `tools/repo_bytecheck.py` đã viết cùng lượt sửa lỗi 403 của nó. Còn 11 file trong `scripts_v1/` và 5 file trong `pipeline/`. Khuôn đã kiểm chứng: lấy nguyên liệu bằng `python tools/docstring_dump.py --scope scripts_v1/`, viết lại tiếng Việt có dấu và không hard wrap mỗi ý một dòng dài, giữ nguyên hậu tố `[KIEM: ...]` ở dòng cuối vì `tools/scripts_index.py` đọc nhãn đó, rồi sinh spec bằng script tự trích neo docstring bằng `ast.get_source_segment`, và bắt buộc xử lý ca docstring raw string vì ba file trong `tools/` dùng khuôn đó. Tiêu chí xong: `python tools/py_audit.py --brief` cho 0 `LOI`. Việc kèm theo: docstring mới dài hơn bản cũ nên `docs/scripts.md` sẽ phình, nếu nó chạm trần 40 KB thì chọn một trong hai hướng, nới trần riêng cho nó trong `PER_FILE_BUDGET` của `tools/docs_audit.py`, hoặc cắt cột mô tả trong `tools/scripts_index.py` xuống câu đầu.
