...
**`tools/docs_patch.py`, tool vá tài liệu dùng chung.** Thay hẳn lối sinh script vá ad-hoc mỗi phiên. Lý do có bằng chứng: năm chốt của script vá là quy ước được chép tay lại mỗi phiên, và riêng ngày 04/08 phần khung đó hỏng hai lần theo hai kiểu khác nhau, một lần anchor viết thiếu dấu tiếng Việt và một lần escape `\n` bị nhân đôi khi đi qua here-string PowerShell rồi lại qua chuỗi Python. Tách dữ liệu vá sang JSON bỏ được đúng tầng escape đã gây lỗi, vì `json.loads` khi đó là tầng escape duy nhất; và nó cũng bỏ luôn lối viết anchor bằng dãy `\u0111` không đọc được bằng mắt, thứ làm hỏng đúng khâu cần kiểm bằng mắt. Thiết kế: nhận một file đặc tả JSON qua `--spec`, mặc định chạy thử không ghi, thêm `--apply` mới ghi. Đặc tả phải phủ đủ năm thao tác mà lượt vá 04/08 đã dùng, thiếu thì tool không thay được script ad-hoc: thay chuỗi, xoá chuỗi tức thay bằng rỗng, chèn thêm giữ nguyên anchor cũ, nối vào cuối file, tạo file mới. Mỗi mục vá khai tên gợi nhớ để dòng log đọc được như `ANCHOR todo-nhan khop=1`. Sáu chốt an toàn bắt buộc: đọc bytes rồi decode và tự dò CRLF hay LF, dừng nếu lẫn, gặp BOM thì báo lỗi chứ không bảo tồn vì luật repo là UTF-8 không BOM; đếm khớp từng anchor bắt buộc bằng đúng 1 và kiểm hết mọi file rồi mới ghi, lệch một chỗ là không sửa file nào kể cả file khác trong cùng lượt đã hợp lệ; so số byte sắp ghi với trần của file đó bằng cách **nhập `PER_FILE_BUDGET` từ `tools/docs_audit.py`** chứ tuyệt đối không khai lại bảng trần trong tool; file `.py` thì `compile()` trước khi ghi; kiểm sau khi ghi bằng `count(new) == 1 and (count(old) == 0 or old in new)`; mặc định từ chối chạy khi `git status` bẩn, có cờ `--allow-dirty`, vì vòng ghi nhiều file không nguyên tử nên cây sạch là thứ duy nhất biến mọi hỏng hóc thành một lệnh `git restore`. Hợp đồng mã thoát khai trong docstring: 0 là xong, khác 0 là chưa ghi gì hoặc ghi xong nhưng kiểm sau thất bại, hai ca phân biệt được bằng số. Tiêu chí xong: tự kiểm đủ ba mã thoát bằng đối chứng dương, đối chứng âm anchor khớp 0 lần, và ca vượt trần, theo lối `data_manifest.py` đã làm; rồi dùng chính nó chạy một lượt vá kết phiên thật. Giới hạn phải ghi vào nhật ký: tool chỉ đảm bảo phần cơ học, anchor viết sai chính tả hay thiếu dấu vẫn sai y như cũ, chỉ khác là bị chặn sạch hơn. File đặc tả đặt ở `data\tmp\` theo khuôn `tmp_<YYYYMMDD>_<nhãn>.json`. Không mô tả tool này trong `START-HERE.md` vì file đó đã 94% trần.
