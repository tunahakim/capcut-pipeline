...
**Sửa 136 lỗi mã hoá và docstring mà `tools/py_audit.py` vừa đo được.** Tool đã có từ 05/08/2026 nên việc còn lại là sửa, không phải nghĩ lại; số đo ở `STATE.md`. Ba việc theo thứ tự. Một, thêm tuỳ chọn loại trừ `_deprecated/` cho `tools/py_audit.py` rồi đo lại, vì thư mục đó là script đã chết và đang chiếm phần lớn con số 136, khiến không ai biết nợ thật to bằng nào. Hai, đối chứng nhánh đoán hard wrap trước khi tin nó: nó đang báo cả `tools/docs_audit.py` lẫn `tools/docs_patch.py` vốn vừa nghiệm thu sạch, nên hoặc phép đoán quá nhạy với dòng cú pháp lệnh, hoặc hai file đó hard wrap thật; chốt bằng cách đọc mắt đúng hai chỗ nó chỉ. Ba, sửa theo nhóm chứ đừng sửa từng file: nhóm `open()` thiếu `encoding`, nhóm `subprocess` bắt output thiếu `encoding`, nhóm thiếu `sys.stdout.reconfigure`, nhóm docstring không dấu. Việc chuyển docstring sang tiếng Việt có dấu và bỏ hard wrap gộp vào đây chứ không còn là nợ nhỏ riêng, vì bảng tra viết không dấu thì người dùng không đọc được và `--find` mất một nửa giá trị. Tiêu chí xong: `python tools/py_audit.py --brief` cho 0 `LOI` trên `tools/` và `scripts_v1/`, và một script in tiếng Việt qua pipe không còn lỗi. Gộp luôn nợ `reference/describe.json` đang là UTF-16LE có BOM: grep xem script nào đọc nó, chuyển sang UTF-8 không BOM, rồi chạy lại chính script ấy.
