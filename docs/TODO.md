...
**`tools/rlog_index.py` sinh `docs/research-log/INDEX.md` tự động, cộng một lượt chèn ngược.** Bảng mục lục viết tay đã hỏng ít nhất một dòng và sẽ hỏng nữa, vì nó là bản thứ hai của một sự thật đã nằm trong từng file nhật ký. Nguồn sự thật là chính file nhật ký: mỗi file khai một dòng `**Tóm tắt:**` ngay dưới tiêu đề H1, tool đọc mọi file, suy ngày và số phiên **từ tên file** chứ không gõ tay, sắp mới nhất lên đầu, ghi lại bảng và giữ nguyên đoạn mở đầu phía trên cùng mọi thứ phía dưới. Chốt để không phải sửa 25 file cũ: file chưa có dòng `**Tóm tắt:**` thì giữ nguyên ô cũ đọc từ bảng hiện có và in `GIU O CU` cho từng file như vậy. Cột Phiên đổi sang dạng máy suy được là `04/08-3`, bỏ hẳn sáng chiều tối vì đó chính là chỗ dòng 04/08-2 đã lệch. Cần thêm một lượt chạy một lần chèn ngược tóm tắt và số phiên từ bảng vào đầu các file nhật ký cũ; gộp lượt đó vào `tools/rlog_index_trim.py` được, hai tool không chồng việc vì trim chỉ lo ngưỡng 30 dòng. Tiêu chí xong: xoá tay một dòng trong bảng rồi chạy tool thì dòng đó hiện lại đúng nguyên văn, và không file nhật ký nào bị sửa thân bài.

**Nâng PowerShell rồi bỏ luật console in ASCII không dấu.** Luật đó ở mục 8 của `docs/START-HERE.md` tồn tại chỉ vì cp1252 của PowerShell 5.1, và nó là động cơ khiến trợ lý học bài học sai rồi viết tiếng Việt không dấu vào cả file tài liệu. Bỏ nguyên nhân thì bỏ được luật. Thứ tự: cài PowerShell 7 bằng winget có ghim phiên bản, kiểm `$OutputEncoding` và code page thật, chạy lại một script in tiếng Việt có dấu ra console rồi so từng ký tự, chỉ khi sạch mới sửa luật. Tiêu chí xong: một script in được câu tiếng Việt có dấu ra console không hỏng ký tự nào, và mục 8 đổi thành bắt buộc có dấu ở mọi nơi.

**Ba nợ của `tools/docs_patch.py`, lộ ra khi dùng thật ngày 04/08.** Nó đọc `PLANNED` từ bản `tools/docs_audit.py` trên đĩa nên một lượt vừa thêm entry vừa nhắc file mới sẽ bị chặn oan, phải tách hai spec; sửa bằng cách đọc `PLANNED` từ bản đã vá trong bộ nhớ, đúng lối `budgets_from_text` đã làm cho bảng trần. Op `delete` đòi nguyên văn cả đoạn nên xoá một mục dài phải dùng script riêng; thêm op `delete_block` nhận anchor dòng đầu rồi xoá tới dòng trống kế tiếp. Và thêm `CANH BAO` khi nội dung mới ghi vào file `.md` dài quá một ngưỡng mà không chứa ký tự có dấu nào, để bắt đúng kiểu hỏng đã làm lệch `docs/research-log/INDEX.md`. Tiêu chí xong: ba ca thử, mỗi ca một dự đoán chốt trước, cộng nghiệm thu mã thoát 3 hiện **chưa kiểm chứng**.
...
**Op `replace_between` dùng sentinel đầu và cuối cho `tools/docs_patch.py`.** Hiện muốn thay một đoạn dài phải dán nguyên văn cả đoạn cũ, tốn token và chính là lý do lượt vá cuối phiên hay hụt ngủ cảnh. Thiết kế: `start` và `end` mỗi cái phải khớp đúng 1 lần, `start` phải đứng trước `end`, và đạc tả phải khai trước số byte dự kiến bị thay để lệch quá một biên độ thì dừng, vì đó là dấu hiệu sentinel cuối bắt vào chỗ xa hơn dự tính. Tiêu chí xong: ba ca thử gồm dương, sentinel cuối khớp hai lần, và ca lệch số byte dự kiến.
