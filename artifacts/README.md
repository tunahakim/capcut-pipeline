...
Bản chụp cột Phiên nguyên bản của `docs/research-log/INDEX.md`, gồm 27 dòng, lưu ngày 05/08/2026 trước khi `python tools/rlog_index.py --apply` sinh lại bảng. Đây là nguồn **duy nhất** của thông tin buổi: 26 trên 28 file nhật ký chưa có dòng khai Phiên, nên bảng là chỗ duy nhất còn giữ các giá trị như `04/08 khuya` hay `31/07 tối muộn`. Lệnh `--apply` sẽ viết lại cả 27 dòng cột Phiên theo khuôn `04/08-4` suy từ tên file và xoá mất phần buổi. Dùng file này làm đầu vào cho lượt chèn ngược dòng khai Phiên, phần Một của mục `tools/rlog_index.py` trong `docs/TODO.md`. Xong việc đó thì file này hết vai trò nhưng vẫn giữ lại làm bằng chứng.