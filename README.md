...
Trần mặc định của mỗi file trong `docs/` là 26 KB, kèm ba lớp ngoại lệ khai tường minh trong `PER_FILE_BUDGET`: `docs/STATE.md` cùng `docs/TODO.md` chật hơn để chống phình, còn `docs/scripts.md`, `docs/reference.md`, `docs/failures.md` được nới lên 40 KB vì chúng thuộc tầng dán chứ không đi qua công cụ fetch. Vì sao mỗi lớp có trần đó, và vì sao trần đo bằng byte trong khi công cụ fetch cắt theo token, xem `docs/ai-reading-channel.md`. Kiểm bằng `python tools/docs_audit.py`, và từ 03/08/2026 lệnh đó **báo lỗi** khi có file vượt trần chứ không chỉ in nhãn, nên `--baseline` không còn chốt được một mốc chuẩn bẩn. Muốn cho một file vượt trần thì thêm trần riêng tường minh cho nó vào `PER_FILE_BUDGET` kèm lý do, để việc vượt trần là quyết định có ghi lại. Đo 03/08/2026 cho thấy nội dung người dùng **dán thẳng vào hội thoại thì không bị cắt**, chỉ kết quả của công cụ fetch mới bị, nên trần 26 KB nay là chốt chống phình chứ không còn là ranh giới sinh tử; cơ chế đầy đủ, ngữ cảnh áp dụng và mô hình đọc ba tầng ở `docs/ai-reading-channel.md`.
